...
createWordNetTree = function(corpus, options) {
  var fDocTrees, fPrunedDocTrees, synsetArray, wordTreshold;
  ...
  fDocTrees = synsetArray.map((function(_this) {
  ...
  BPromise.all(fDocTrees).then((function(_this) {
    return function(docTrees) {
      console.timeEnd("Step 2: Generate Candidate Sets");
      return console.time("Step 3: Pruning (Word Sense Disambiguation)");
    };
  })(this));
  fPrunedDocTrees = BPromise.all(fDocTrees).filter((function(_this) {
    return function(doc) {
      return doc !== null;
    };
