#!/usr/bin/env node;
var BPromise, calculateCounts, constructSynsetData, createWordNetTree, csv, fs, generateCorpusTree, generatePDF, generateWordTree, getCorpusSynsets, mime, pickSynsets, prepareWordnetTree, program, thresholdDocTree, thresholdTree, thresholdWordTree, util, writePDF, _ref, _ref1;
...
createWordNetTree = function(corpus, options) {
  ...
  docTrees = synsetArray.map((function(_this) {
    return function(d, index) {
      ...
      return wordTrees.filter(function(word) {
        return word !== null;
      });
    };
  })(this));
  ...
  fPrunedDocTrees = docTrees.filter((function(_this) {
    return function(doc) {
      return doc !== null;
    };
...
generatePDF = function(options) {
  var file, synsetTree, writeStream;
...
program.command('PDF').description('generate pdf report').option('-i, --input [value]', 'Input JSON synset tree file').option('-o, --output [value]', 'File name of generated PDF').action(generatePDF);
