#!/usr/bin/env node;
var BPromise, ProgressBar, calculateCounts, child_process, cluster, constructSynsetData, createWordNetTree, csv, fs, generateCorpusTree, generatePDF, generateWordTree, getCorpusSynsets, mime, pickSynsets, prepareWordnetTree, program, querystring, rp, thresholdDocTree, thresholdTree, thresholdWordTree, util, writePDF, _ref, _ref1;
...
createWordNetTree = function(corpus, options) {
  var fPrunedDocTrees, progressGetBestSynsets, synsetArray, wordTreshold;
  ...
  progressGetBestSynsets = new ProgressBar('synset disambiguation [:bar] :percent :etas', {
    total: synsetArray.length
  });
  fPrunedDocTrees = synsetArray.map((function(_this) {
    return function(d, index) {
      ...
      return res = rp.post('http://localhost:8000/getBestSynsets', {
