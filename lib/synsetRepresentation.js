(function() {
  var BPromise, DICTIONARY, EXCEPTIONS, MORPHY_SUBSTITUTIONS, WORDNETIFY_SYNSETS_TREE, Word, arr, createDocTree, exports, fs, getCorpusSynsets, getWordSynsets, logger, memoize, morphy, str, tm, util, _,
  ...
  Word = (function() {
    ...
    Word.prototype.getSynsets = function(callback) {
      var contained, correctPOS, insert_synset, isContained, key, ret, synset;
      ret = [];
      ...
      for (key in WORDNETIFY_SYNSETS_TREE) {
        if (!__hasProp.call(WORDNETIFY_SYNSETS_TREE, key)) continue;
  ...
  getCorpusSynsets = function(docs) {
    ...
    corpus = corpus.removeInterpunctuation().removeNewlines().toLower().clean().removeWords(tm.STOPWORDS.EN).clean().removeDigits();
