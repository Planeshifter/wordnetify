(function() {
  ...
  writePDF = function(output, filename) {
  ...
  writeDocPdfReport = function(output, filename) {
    return console.log('Should write DOC Reports');
  };

  writeCorpusPdfReport = function(output, filename) {
    ...
    printSynset = function(synset, depth) {
      ...
      })(this)).splice(0, 3).join(", ") + (" (" + synset.wordCount + "):"));
      doc.fillColor('black');
      doc.fontSize(6);
